...
- Основная таблица показывает только точные предложения. Контекстное меню строки по правой кнопке мыши и доступная кнопка строки содержат действие «Показать аналоги»; оно открывает отдельное модальное окно с компактной карточкой выбранной позиции, потоковым прогрессом, поиском по результатам, сортировкой, настройкой наценки и таблицей аналогов. Окно имеет одну область прокрутки результатов и сохраняет клавиатурный фокус внутри до закрытия.
