...
- Основная таблица показывает только точные предложения и использует тот же comparison-oriented интерфейс, что и аналоги: компактную панель поиска и наценки, счетчик, фиксированные колонки, закрепленную шапку, единую область прокрутки и выделение лучшей цены. Контекстное меню строки по правой кнопке мыши и доступная кнопка строки содержат действие «Показать аналоги»; оно открывает отдельное модальное окно с компактной карточкой выбранной позиции, потоковым прогрессом, поиском по результатам, сортировкой, настройкой наценки и таблицей аналогов. Окно сохраняет клавиатурный фокус внутри до закрытия.
