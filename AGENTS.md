...
## Внешняя база знаний AIOS

Для этого проекта подключена постоянная база знаний:

`E:\YandexDisk\Work\library\AIOS`

В начале каждой задачи агент обязан:

1. Прочитать `E:\YandexDisk\Work\library\AIOS\AGENTS.md`.
2. Прочитать `E:\YandexDisk\Work\library\AIOS\WIKI\index.md`.
3. Открыть `WIKI/projects/autoservice-aggregator.md` и минимальный набор других страниц, связанных с текущей задачей.
4. Не загружать всю базу или весь `RAW` без конкретной необходимости.

Текущие инструкции пользователя, этот `AGENTS.md` и актуальные файлы репозитория авторитетнее сохранённых заметок AIOS. При расхождении нужно проверить текущее состояние проекта и не переносить устаревшее знание в код.

Если задача создаёт подтверждённое долговечное знание или пользователь явно просит что-либо запомнить, следуй правилам записи из AIOS `AGENTS.md`: не сохраняй секреты, избегай дублирования и используй `knowledge_curator`, когда он доступен. Недоступность пути или куратора не должна молча игнорироваться: продолжи безопасную работу по актуальным файлам проекта и укажи ограничение в итоговом отчёте.
...
Отсутствие ключей Rossko API должно приводить к понятной ошибке конфигурации или авторизации, а не к mock-результатам.
...
- После успешного завершения задачи с изменениями автоматически создай отдельную ветку `codex/<краткое-описание>`, commit и push этой ветки в `origin`, если пользователь явно не запретил Git-операции. Запрос пользователя на изменение проекта считается разрешением на этот безопасный workflow; отдельное подтверждение для ветки, commit и push не требуется.
- Если работа началась в `main` или другой основной ветке, создай `codex/...` до staging. Не выполняй commit непосредственно в `main` и не отправляй изменения в `origin/main`.
- Включай в commit только файлы текущей задачи. Перед commit проверь `git status --short` и `git diff --cached`; несвязанные изменения рабочего дерева оставь нетронутыми и не добавляй в staging.
- Создавай commit только после обязательных проверок проекта. Если проверки не прошли, commit и push не выполняй и явно сообщи причину.
- Отправляй ветку командой, эквивалентной `git push -u origin <ветка>`, чтобы upstream был настроен и ветку можно было получить на другом компьютере через `git fetch origin` и `git switch --track origin/<ветка>`.
- Не выполняй автоматическое слияние в `main`, rebase опубликованной ветки, force-push, удаление remote-ветки, создание tag или pull request без отдельного явного запроса.
- Если защитная политика среды требует разрешения на запись в `.git` или сетевой push, запрашивай минимальное узкое разрешение только для необходимой Git-команды. Не отключай и не обходи системную политику безопасности.
