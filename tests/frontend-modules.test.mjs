...
test("main-search filters use a compact trigger, supplier disclosure, and direct filter buttons", async () => {
  ...
  assert.match(app, /Math\.max\(50, Math\.round\(width \/ 10\) \* 10\)/);
  ...
  assert.match(styles, /\.filters-sidebar\s*\{[^}]*--filters-sidebar-width: 200px;[^}]*min-width: 50px;/s);
  ...
  assert.match(app, /const filtersCloseThresholdRatio = 0\.05;/);
